...
import { 
  ...
  Clock, Gift, Cake, Moon, Sun, Camera, Eye
} from 'lucide-react';
...
export default function Home() {
  ...
  const startDemo = (occasion: 'birthday' | 'anniversary' | 'graduation' | 'classic') => {
    const configs: Record<string, any> = {
        birthday: {
            ...
            spotifyTracks: { "day0": "5In99v09ZvxrkZST00Id06", "day1": "60S9Z396v9ZvxrkZST00Id06" },
  ...
  if (!isRealSanctuary || isPreviewing) {
    if (isPreviewing || !isRealSanctuary) {
        return (
          <main className="min-h-screen bg-[#FDFCFB] text-slate-900 overflow-x-hidden selection:bg-sanctuary-secondary selection:text-sanctuary-primary">
            <AnimatePresence>
                {isPreviewing && (
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[2000] bg-white overflow-y-auto">
                        <div className="fixed top-6 left-1/2 -translate-x-1/2 w-[90%] max-w-2xl glass rounded-2xl flex justify-between items-center p-3 z-[3000] shadow-2xl border border-white/40">
            ...
            <nav className="fixed top-0 left-0 w-full p-6 flex justify-between items-center z-[100] backdrop-blur-sm bg-white/30 border-b border-black/5">
                <div className="flex items-center gap-2">
                    ...
                    <span className="font-serif-display text-xl tracking-tighter">Sanctuary</span>
                </div>
            ...
            <header className="min-h-screen flex flex-col items-center justify-center px-6 pt-20 relative text-center">
                ...
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="max-w-4xl space-y-8">
                    <div className="inline-flex items-center gap-2 px-4 py-2 glass rounded-full text-[10px] font-bold uppercase tracking-[0.2em] text-sanctuary-primary mb-4 shadow-sm border-white/60">
                        <Sparkles size={12} className="animate-pulse" />
                        A New Way to Give
                    </div>
                    <h1 className="text-7xl md:text-9xl font-serif-display tracking-tight text-slate-900 leading-[0.9]">
                        Don't just give. <br /> <span className="text-sanctuary-primary italic pr-4">Build a Sanctuary.</span>
                    </h1>
            ...
            <section id="bento" className="py-32 px-6 bg-slate-50/50 relative overflow-hidden">
                <div className="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-12 gap-6 auto-rows-[240px]">
                    ...
                    <div className="md:col-span-6 lg:col-span-4 bg-slate-900 p-10 rounded-[40px] shadow-2xl flex flex-col justify-between text-white relative overflow-hidden group">
                        <div className="relative z-10 space-y-4">
                            <div className="w-10 h-10 bg-white/10 rounded-xl flex items-center justify-center"><Shield size={20} className="text-sanctuary-secondary" /></div>
                            <h3 className="text-3xl font-serif-display">Private by Design</h3>
                            <p className="text-white/60 text-sm leading-relaxed">No database. Zero tracking. Your memories are encrypted and live only in your unique link.</p>
                        </div>
                    ...
                    <div className="md:col-span-6 lg:col-span-4 bg-white p-10 rounded-[40px] shadow-sm border border-black/[0.03] flex flex-col justify-between overflow-hidden group relative">
                        <div className="space-y-4">
                            <h3 className="text-3xl font-serif-display">Tactile Memories</h3>
                        ...
                        <div className="w-full h-32 bg-slate-100 rounded-2xl relative overflow-hidden">
                            ...
                            <div className="absolute top-0 left-0 w-1/2 h-full bg-white group-hover:translate-x-full transition-transform duration-1000" />
                        </div>
                    </div>
                    ...
                    <div className="md:col-span-12 lg:col-span-8 bg-[#F5F3FF] p-10 rounded-[40px] shadow-sm border border-black/[0.03] flex flex-col justify-between group overflow-hidden relative">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-center h-full">
                            <div className="space-y-4">
                                <h3 className="text-3xl font-serif-display">Premium Cinema</h3>
                                ...
                                <div className="flex gap-2">
                                    {[1, 2, 3, 4].map(i => <div key={i} className="w-8 h-10 bg-white rounded-lg border border-black/5 shadow-sm" />)}
                                </div>
                            </div>
            ...
            <footer className="py-20 border-t border-black/5 bg-slate-50/50">
                <div className="max-w-6xl mx-auto px-6 grid grid-cols-1 md:grid-cols-2 gap-12 text-slate-400">
                    <div className="space-y-6">
                        ...
                        <p className="text-xs max-w-xs italic leading-relaxed">"I built this to turn digital gifts into emotional sanctuaries. Built with love in Denmark, for lovers everywhere."</p>
                    </div>
                    <div className="flex gap-12 text-[10px] uppercase font-bold tracking-[0.2em] justify-end items-end">
                        <Link href="/privacy" className="hover:text-slate-900">Privacy</Link>
                        <Link href="/terms" className="hover:text-slate-900">Terms</Link>
                        <Link href="/revoke" className="hover:text-red-500">Revoke</Link>
                        <a href="mailto:malthe@mbn-code.dk" className="hover:text-slate-900 lowercase font-sans">malthe@mbn-code.dk</a>
                    </div>
                </div>
            </footer>
          </main>
