...
import { 
  ...
  Plus, Trash2, FileText, Upload, Shield, Cake, Gift, Bell, GraduationCap, Baby, PartyPopper, CheckCircle2, User
} from 'lucide-react';
  ...
  const sendEmail = async () => {
    ...
    try {
        ...
        if (res.ok) {
            ...
            alert("Failed to send email. Check the address.");
        }
    } catch (e) {
        console.error(e);
    } finally {
      ...
      <div className="max-w-3xl w-full bg-white rounded-[40px] shadow-xl overflow-hidden flex flex-col min-h-[650px] border border-black/[0.03]">
        <div className="bg-sanctuary-primary p-8 text-white flex items-center justify-between">
          <div className="flex items-center gap-4 text-left">
            ...
            <div>
                <h1 className="text-xl font-serif-display tracking-tight">Magic Gift</h1>
        ...
        <div className="flex-grow p-10 overflow-y-auto custom-scrollbar bg-[#FDFCFB]/50">
            ...
            <motion.div key={step} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-8">
                ...
                <div className="space-y-8 text-center">
                    ...
                    <div className="bg-white p-6 rounded-[32px] border border-black/[0.03] shadow-sm text-left space-y-4">
                        <ul className="grid grid-cols-1 gap-3">
                            <li className="flex items-center gap-3 text-xs text-slate-600 font-medium"><div className="w-5 h-5 bg-green-50 rounded-full flex items-center justify-center text-green-600"><Check size={12} /></div> {config.plan === 'spark' ? '5 Messages' : config.plan === 'plus' ? '25 Messages' : '500 Messages'}</li>
                            <li className="flex items-center gap-3 text-xs text-slate-600 font-medium"><div className="w-5 h-5 bg-green-50 rounded-full flex items-center justify-center text-green-600"><Check size={12} /></div> {config.plan === 'spark' ? '1 Day Countdown' : config.plan === 'plus' ? '7 Day Countdown' : '14 Day Journey'}</li>
                            <li className="flex items-center gap-3 text-xs text-slate-600 font-medium"><div className="w-5 h-5 bg-green-50 rounded-full flex items-center justify-center text-green-600"><Check size={12} /></div> {config.plan === 'spark' ? '10 Photos' : config.plan === 'plus' ? '30 Photos' : '50 Photos'}</li>
                        </ul>
                    </div>
              ...
              {step === 2 && (
                <div className="space-y-8 text-left">
                  <div className="space-y-4">
                    <label className="block text-xs font-bold text-slate-400 uppercase tracking-[0.2em]">Select Occasion</label>
                  ...
                  <div className="space-y-4 pt-4 border-t border-black/[0.03]">
                    <label className="block text-xs font-bold text-slate-400 uppercase tracking-[0.2em]">Customize Names</label>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div className="space-y-2">
                            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">From</p>
                            <input type="text" value={config.names.sender} onChange={(e) => updateConfig('names.sender', e.target.value)} placeholder="Alex" className="w-full p-4 rounded-2xl border-2 border-black/[0.03] focus:border-sanctuary-primary outline-none transition-colors bg-white shadow-sm text-gray-800" />
                        </div>
                        <div className="space-y-2">
                            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">To</p>
                            <input type="text" value={config.names.recipient} onChange={(e) => updateConfig('names.recipient', e.target.value)} placeholder="Sam" className="w-full p-4 rounded-2xl border-2 border-black/[0.03] focus:border-sanctuary-primary outline-none transition-colors bg-white shadow-sm text-gray-800" />
                        </div>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-4">
                    <div className="space-y-2 text-left">
                        <label className="block text-xs font-bold text-slate-400 uppercase tracking-[0.2em]">Event Date</label>
                        <input type="date" value={config.targetDate.split('T')[0]} onChange={(e) => updateConfig('targetDate', new Date(e.target.value).toISOString())} className="w-full p-4 rounded-2xl border-2 border-black/[0.03] focus:border-sanctuary-primary outline-none bg-white shadow-sm text-gray-800" />
                    </div>
                    <div className="space-y-2 relative text-left">
                        <label className="block text-xs font-bold text-slate-400 uppercase tracking-[0.2em]">Duration (Days)</label>
                        <input type="number" min={1} max={currentLimits.days} value={config.totalDays} onChange={(e) => { const val = parseInt(e.target.value); if (val <= currentLimits.days) { updateConfig('totalDays', val); } }} className="w-full p-4 rounded-xl border-2 border-black/[0.03] focus:border-sanctuary-primary outline-none bg-white shadow-sm text-gray-800" />
                        <p className="text-[10px] text-slate-400 mt-1 italic">Plan limit: {currentLimits.days} days</p>
                    </div>
                  </div>
                  
                  <div className="space-y-4 pt-4 border-t border-black/[0.03]">
                    <label className="block text-xs font-bold text-slate-400 uppercase tracking-[0.2em]">Custom Question</label>
                    <input type="text" value={config.customQuestion || ""} onChange={(e) => updateConfig('customQuestion', e.target.value)} placeholder="Will you be my Valentine?" className="w-full p-4 rounded-2xl border-2 border-black/[0.03] focus:border-sanctuary-primary outline-none bg-white shadow-sm text-gray-800" />
                  </div>

                  <div className="space-y-4 pt-4 border-t border-black/[0.03]">
                    <label className="block text-xs font-bold text-slate-400 uppercase tracking-[0.2em]">Custom Background Image</label>
                    <div className={`relative rounded-xl border-2 border-dashed p-6 transition-all border-black/[0.05] bg-white hover:border-sanctuary-primary cursor-pointer`}>
                          ...
                          return (
                            <div key={offset} className="p-6 bg-white rounded-[32px] border border-black/[0.03] shadow-sm space-y-4">
                              ...
                              <div className="space-y-4 text-left">
                                <label className={`flex flex-col items-center justify-center gap-2 border-2 border-dashed border-black/[0.05] rounded-2xl p-8 transition-all hover:border-sanctuary-primary cursor-pointer bg-slate-50 group ${uploading ? 'pointer-events-none' : ''}`}>
                                ...
                                {images.length > 0 && (
                                    <div className="grid grid-cols-3 gap-2 mt-4 text-left">
                                        {images.map((url, idx) => (
                                            <div key={idx} className="relative group rounded-xl overflow-hidden border border-black/[0.05] aspect-square bg-white shadow-sm text-left">
              ...
              {step === 5 && (
                <div className="space-y-6 text-left text-gray-800">
                  <div className="space-y-2">
                    ...
                    <p className="text-sm text-slate-500 font-playfair italic">Whisper sweet messages that reveal themselves over time.</p>
                  </div>
                    ...
                    {config.notes.map((note, idx) => (
                        <div key={note.id} className="p-5 bg-white rounded-[32px] border border-black/[0.03] shadow-sm space-y-3 group hover:border-sanctuary-secondary transition-all">
                            <div className="flex justify-between items-center">
                                <select value={note.day} onChange={(e) => { const newNotes = [...config.notes]; newNotes[idx].day = parseInt(e.target.value); updateConfig('notes', newNotes); }} className="p-2 rounded-lg border border-black/[0.05] bg-slate-50 text-[10px] font-bold uppercase tracking-widest text-slate-500 outline-none focus:border-sanctuary-primary">{getDaysArray().map(offset => ( <option key={offset} value={offset}>{offset === 0 ? "The Big Day" : `${offset} Days Left`}</option> ))}</select>
                                {config.notes.length > 1 && ( <button onClick={() => { const newNotes = config.notes.filter((_, i) => i !== idx); updateConfig('notes', newNotes); }} className="text-red-200 hover:text-red-500 transition-colors"><X size={14} /></button> )}
                            </div>
                            <textarea value={note.content} onChange={(e) => { const newContent = e.target.value; const otherLen = config.notes.reduce((acc, n, i) => i === idx ? acc : acc + (n.content?.length || 0), 0); if (otherLen + newContent.length <= 8000) { const newNotes = [...config.notes]; newNotes[idx].content = newContent; updateConfig('notes', newNotes); } }} placeholder="Write your message here..." className="w-full p-3 bg-slate-50/50 rounded-xl border border-black/[0.03] focus:border-sanctuary-primary outline-none text-sm min-h-[80px] resize-none text-gray-800" />
                        </div>
                    ))}
                    <button onClick={() => { const newNotes = [...config.notes, { id: `note${Date.now()}`, day: 0, content: '' }]; updateConfig('notes', newNotes); }} className="w-full py-4 border-2 border-dashed border-black/[0.05] text-slate-400 rounded-[32px] font-bold hover:bg-slate-50 transition-all text-xs uppercase tracking-widest">+ Add Another Message</button>
                  </div>
                </div>
              )}
                ...
                <div className="space-y-6 text-left text-gray-800">
                  {!currentLimits.video ? (
                    <div className="p-12 text-center bg-sanctuary-primary/5 rounded-[40px] border-2 border-dashed border-sanctuary-secondary/30">
                        <ImageIcon size={48} className="mx-auto text-sanctuary-secondary mb-4 opacity-50" />
                        <h3 className="text-2xl font-serif-display text-slate-900 mb-2">Secret Cinema</h3>
                        <p className="text-sm text-slate-500 mb-8 font-playfair italic">The ultimate finale. Upgrade to <b>The Sanctuary</b> plan to upload your own video.</p>
                        <button onClick={() => setStep(1)} className="px-8 py-3 bg-slate-900 text-white rounded-2xl font-bold shadow-lg block mx-auto hover:scale-105 transition-all text-sm">Upgrade Experience</button>
                    </div>
                  ) : (
                    <div className="space-y-6 text-left">
                      <div className="space-y-2">
                      ...
                      <div className={`relative rounded-[40px] border-2 border-dashed p-10 transition-all border-black/[0.05] bg-white hover:border-sanctuary-primary`}>
                        {config.videoUrl ? (
                            <div className="space-y-4 text-left">
                                <div className="aspect-video w-full rounded-3xl overflow-hidden border border-black/[0.05] bg-black shadow-2xl"><video src={config.videoUrl} className="w-full h-full object-cover" controls /></div>
                                <div className="flex justify-between items-center"><p className="text-[10px] font-bold text-sanctuary-primary uppercase tracking-[0.2em]">Video Secured</p><button onClick={() => { const url = config.videoUrl; updateConfig('videoUrl', ''); if (url) deleteAsset(url); }} className="text-[10px] text-slate-400 underline uppercase font-bold hover:text-red-500">Remove</button></div>
                            </div>
                        ) : (
                            <label className="flex flex-col items-center justify-center gap-6 cursor-pointer text-left">
                                ...
                                <div className="text-center space-y-2"><p className="text-sm font-bold text-slate-900 uppercase tracking-widest">Select Video File</p><p className="text-[10px] text-slate-400 italic">MP4 or MOV • Under 50MB recommended</p></div>
              ...
              {step === 7 && (
                <div className="space-y-8 text-center text-gray-800">
                   <div className="space-y-6 text-center">
                    <div className="w-16 h-16 bg-slate-900 rounded-2xl flex items-center justify-center text-white mx-auto shadow-xl"><Lock size={32} /></div>
                    <div className="space-y-2">
                        <h3 className="text-3xl font-serif-display text-slate-900">Set Your Passcode</h3>
                        <p className="text-sm text-slate-500 font-playfair italic">A 4-digit key to unlock the most private parts of your sanctuary.</p>
                    </div>
                    <div className="max-w-[280px] mx-auto space-y-4">
                        <input type="text" maxLength={4} value={config.passcode} onChange={(e) => updateConfig('passcode', e.target.value.replace(/\D/g, ''))} className={`w-full p-6 text-center text-5xl tracking-[0.5em] font-serif-display rounded-[32px] border-2 transition-all outline-none border-black/[0.03] focus:border-sanctuary-primary focus:bg-white bg-slate-50 shadow-inner text-gray-800`} />
                        <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">Choose 4 numbers</p>
                    </div>
                  </div>
                </div>
              )}
              ...
              {step === 8 && (
                <div className="space-y-8 text-center py-10 text-gray-800">
                  <div className="w-20 h-20 bg-green-50 text-green-600 rounded-full flex items-center justify-center mx-auto mb-4 animate-bounce shadow-xl shadow-green-100"><Check size={40} /></div>
                  <h2 className="text-5xl font-serif-display text-slate-900 tracking-tight text-center">Sanctuary Ready</h2>
                  <div className="space-y-6 text-left">
                    <div className="p-6 bg-slate-50 rounded-[32px] border border-black/[0.03] break-all text-xs font-mono shadow-inner text-slate-500 text-center">{generatedLink}</div>
                    <div className="flex flex-col gap-4">
                      <button onClick={copyToClipboard} className="w-full flex items-center justify-center gap-3 py-5 bg-slate-900 text-white rounded-2xl font-bold shadow-2xl hover:scale-[1.02] transition-all">
                        {copied ? <Check size={20} /> : <Copy size={20} />} {copied ? 'Copied!' : 'Copy Sanctuary Link'}
                      </button>
                      
                      <div className="p-8 bg-slate-50 rounded-[40px] border border-black/[0.03] space-y-6 relative overflow-hidden">
                        <div className="relative z-10 space-y-4">
                            <div className="flex items-center gap-2 text-slate-900 font-bold uppercase tracking-[0.2em] text-[10px]"><Plus size={16} /> Delivery Options</div>
                            
                            {!emailSent ? (
                                <div className="space-y-3">
                                    <p className="text-[11px] text-slate-500 italic">Never lose your link. Send a backup to your email.</p>
                                    <div className="flex gap-2">
                                        <input 
                                            ...
                                            className="flex-grow p-3 rounded-xl border border-black/[0.05] text-xs focus:border-sanctuary-primary outline-none"
                                        />
                                        <button 
                                            onClick={sendEmail}
                                            disabled={isSendingEmail || !userEmail}
                                            className="px-4 py-2 bg-slate-900 text-white rounded-xl font-bold text-[10px] uppercase tracking-widest disabled:opacity-50"
                                        >
                                            {isSendingEmail ? <LucideLoader className="animate-spin" size={14} /> : 'Send'}
                                        </button>
                                    </div>
                                </div>
                            ) : (
                                <div className="flex items-center gap-2 text-green-600 bg-green-50 p-3 rounded-xl border border-green-100">
                                    ...
                                    <span className="text-[10px] font-bold uppercase">Sent to Inbox</span>
                                </div>
                            )}
                        </div>
                      </div>

                      <div className="p-10 bg-[#F5F3FF] rounded-[48px] border border-black/[0.03] space-y-6 relative overflow-hidden">
                        <div className="relative z-10 space-y-4">
                            <div className="flex items-center gap-2 text-indigo-600 font-bold uppercase tracking-[0.2em] text-[10px]"><Sparkles size={16} /> Social Media Pack</div>
                            ...
                            <button onClick={downloadShareCard} disabled={isDownloading} className="w-full py-4 bg-white border border-black/[0.05] text-slate-900 rounded-2xl font-bold text-xs uppercase tracking-widest flex items-center justify-center gap-3 hover:bg-slate-50 transition-all shadow-sm disabled:opacity-50">
                        ...
                        <ThemeIcon className="absolute bottom-[-30px] right-[-30px] text-indigo-600/5 w-48 h-48" />
                      </div>
                      ...
                      <div className="flex justify-center gap-12 pt-6">
                        ...
                        <button onClick={handleDelete} className="text-[10px] text-red-300 uppercase tracking-[0.3em] font-bold hover:text-red-600 transition-colors">Destroy</button>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="fixed left-[-9999px] top-0">
                    <div ref={cardRef} className="w-[1080px] h-[1920px] bg-white flex flex-col items-center justify-center p-20 text-center relative overflow-hidden" style={{ backgroundColor: theme.colors.bg }}>
                        ...
                        <div className="relative z-10 space-y-12">
                            ...
                            <h1 className="text-[120px] font-serif-display tracking-tight text-slate-900 leading-tight">For {config.names.recipient}</h1>
                            <p className="text-[48px] text-slate-400 font-bold uppercase tracking-[0.3em]">Built by {config.names.sender}</p>
                            <div className="pt-24"><div className="px-16 py-8 bg-slate-900 text-white rounded-[40px] text-[48px] font-bold shadow-2xl text-center">MagicGift ✨</div></div>
                        </div>
                        <p className="absolute bottom-20 text-[32px] font-bold text-slate-300 uppercase tracking-[0.4em] text-center">Personalized Sanctuary</p>
                    </div>
                  </div>
                </div>
              )}
        ...
        {/* Footer */}
        {step < 8 && (
          <div className="p-8 bg-slate-50/50 border-t border-black/[0.03] flex justify-between items-center text-gray-800">
            <button onClick={() => setStep(Math.max(1, step - 1))} disabled={step === 1 || !!uploading} className={`flex items-center gap-2 font-bold text-[10px] uppercase tracking-widest transition-all ${step === 1 || !!uploading ? 'text-slate-200' : 'text-slate-400 hover:text-slate-900'}`}>
              <ArrowLeft size={16} /> Prev
            </button>
            <div className="flex gap-3">
                {step < 7 ? (
                    <button onClick={() => setStep(step + 1)} disabled={!!uploading} className="bg-slate-900 text-white px-10 py-4 rounded-2xl font-bold shadow-xl hover:scale-105 transition-all text-[10px] uppercase tracking-[0.2em] disabled:opacity-50">
                        {uploading ? 'Working...' : 'Continue'} <ArrowRight size={16} className="inline ml-1" />
                    </button>
                ) : (
                    <button onClick={handleGenerate} disabled={isPaying || !!uploading} className="bg-sanctuary-primary text-white px-10 py-4 rounded-2xl font-bold shadow-xl hover:scale-105 transition-all text-[10px] uppercase tracking-[0.2em] disabled:opacity-50">
...
function PreviewApp({ forceUpdateKey }: { forceUpdateKey: number }) {
    ...
    return (
        <div className="min-h-screen" key={forceUpdateKey}>
...
export default function WizardPage() {
    return (
        <Suspense fallback={<div className="min-h-screen bg-white flex items-center justify-center"><Sparkles className="text-sanctuary-primary animate-pulse" size={48} /></div>}>
            <WizardContent />
        </Suspense>
    );
}
